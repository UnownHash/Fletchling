...
RUN CGO_ENABLED=0 go build -ldflags="-s -w" -o /go/bin/fletchling-db-exporter ./bin/fletchling-db-exporter
RUN CGO_ENABLED=0 go build -ldflags="-s -w" -o /go/bin/sleep ./bin/sleep
RUN mkdir /empty-dir
